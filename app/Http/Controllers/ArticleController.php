...
class ArticleController extends Controller
{
    ...
    public function list_content() {
        $articles = Article::orderBy('created_at')->paginate(8);
        $carousels = Article::latest()->limit(3)->get();

        return view('layouts.article',compact('articles','carousels')); 
    }

    // public function search(Request $request) {
    //     $search = $_GET['search'];
    //     $articles = Article::where('topic', 'like', '%'.$search.'%')->get();

    //     return view('layouts.article',compact('articles')); 
    //     //$search = $request->get('search');
    //    // $articles = DB::table('articles')->where('topic', 'like', '%'.$search.'%')->paginate(8);
    //     //return view('layouts.article',['posts=>$posts']); 
    // }
